...
public class CompanyShuntOld {
    ...
    private static List<List<CompanyGroup>> findAllCombinations(List<CompanyGroup> companyGroupList, int bestBossNum) {
        ...
        // 初始化dp数组
        boolean[][] dp = new boolean[len + 1][bestBossNum + 1];
        dp[0][0] = true;
        for (int i = 1; i <= len; i++) {
            dp[i][0] = true;
        }
        ...
        for (int i = 1; i <= len; i++) {
            for (int j = 1; j <= bestBossNum; j++) {
                if (j < companyGroupList.get(i - 1).getBossNum()) {
                    ...
                    dp[i][j] = dp[i - 1][j] || (i == 1 || companyGroupList.get(i - 2).getBossNum() != companyGroupList.get(i - 1).getBossNum()) && dp[i - 1][j - companyGroupList.get(i - 1).getBossNum()];
                }
            }
        }
        ...
        for (int n = len; n >= 0; n--) {
            ...
            for (int i = n; i > 0 && remainingSum > 0; i--) {
                if (remainingSum >= companyGroupList.get(i - 1).getBossNum() && dp[i - 1][remainingSum - companyGroupList.get(i - 1).getBossNum()] && (i == 1 || companyGroupList.get(i - 2).getBossNum() != companyGroupList.get(i - 1).getBossNum())) {
